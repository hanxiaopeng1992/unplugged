...
\markboth{Natural numbers}{Mathematics in Computer Programming}
...
Euclid's {\em Element} is the first work introduced the axiomatic methods. From the five axioms and five postulates, Euclid developed the laws one by one elaborately. Every result is only based on the axioms and the laws proved before. With this approach, he built the great building of geometry. However, theres no axiomatic formal system for natural numbers for long time. People have been considering the natural number is straightforward and the related facts are obvious. The axioms of natural number was setup by Italian mathematician Peano till 1889\footnote{Peano is a mathematician, logician, and linguist. He is the pioneer of mathematical logic and set theory. Peano influenced Russel and Whitehead for their work {\em Principia Mathematica}, and the further programs of French Bourbaki school. Peano also invented an international auxiliary language called Latino sine flexione ("Latin without inflexion," later called Interlingua, and the precursor of the Interlingua of the IALA).}. Today, we called Peano Axioms. Surprised similar to Euclid's five geometry axioms, there are five Peano natural number axioms.
...
This scheme is called as {\em structural} recursion over the natural numbers. Below examples show how it behaves over the first
 several numbers.
...
\item Define sum of squares for odd numbers, what sequence does it produce?
...
\item Read in a sequence of digits (string of digit numbers), convert it to decimal. How to handle hexadecimal digit and number? How to handle the decimal point?
