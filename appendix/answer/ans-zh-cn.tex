...
\[
(n + 1)^m = n^m + \binom{n}{1} n^{m-1} + ... + \binom{n}{n-1} n + 1
\]
...
这里$C \cdot X$是二项式系数和各次幂的点积$C \cdot X = \sum_{j} c_j x_j$。各次幂通过$X = take\ (m + 1)\ (iterate (y \mapsto y / i)\ x)$求出，二项式定理的系数可以由帕斯卡三角形逐行递推得到：
...
其中：
\bre
gen\ C\ [x_1, x_2, x_3, ...] & = & gen\ ((x_1 + x_2) : C)\ [x_2, x_3, ...] & \\
gen\ C\ X & = & (1:C) & \text{$X$不足两个元素时} \\
\ere
