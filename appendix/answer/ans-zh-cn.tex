...
\item 奇偶判断函数在整数加群$(Z，+)$和布尔逻辑与群$(Bool, \land)$下是否构成同态？去除0元素的整数乘法群呢？
...
事实上，第二、三种情况的伽罗瓦群在其分裂域上是同构的。我们注意到$f(f(x)) = x$所以它同构于只有两个元素${0, 1}$的模2加群，也同构于循环群$C_2$或$Z/2Z$。其中$Z/2Z$这个符号表示整数加群$Z$和其偶数子群$2Z$的商群。

\item {证明，如果$p$是素数，则方程$x^p - 1$的伽罗瓦群是$p-1$阶的循环群$C_{p-1}$。}

方程$x^p -1$的$p$个根就是在复平面单位元上的$p$个点$1, \omega, \omega^2, ..., \omega^{p-1}$，其中$\omega = e^{2 \pi i / p}$。其分裂域是$Q[\omega]$, 我们构造这样的一组$Q-$自同构$\{h_1, h_2, ..., h_{p-1}\}$，其中：

\[
h_i(\omega) = \omega^i
\]

显然$h_i(\omega)$也是方程的根。我们现在证明，这$p-1$个自同构组成的伽罗瓦群$Gal(Q[\omega]/Q)$是一个循环群。
