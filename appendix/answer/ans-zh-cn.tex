...
根据前一道题目，我们知道方程的根是复平面单位元上的5个点：\{1, $\zeta$, $\zeta^2$, $\zeta^3$, $\zeta^4$ \}。其中$\zeta = e^{\dfrac{\pi}{5} i} = \dfrac{\sqrt{5} - 1 + \sqrt{10 + 2 \sqrt{5}}}{4}$。在有理数域上的伽罗瓦群是一个4阶循环群$G(Q[\zeta]/Q) = C_4$，同构于模5乘法群：$(\pmb{Z}/5\pmb{Z})^{\times} = \{1, 2, 3, 4 \}_5$。没有中间扩域。其分裂域为$Q[\zeta]$，在分裂域上的伽罗瓦群只有单位元${1}$。

显然${1}$是$C_4$的正规子群。并且其商群$C_4/{1}$是阿贝尔群。因此方程是根式可解的。
