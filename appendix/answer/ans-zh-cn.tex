...
所以，$f(\omega)$也是某个$p$次单位根（方程$x^p - 1 = 0$的某个根\footnote{如果$p$不是素数，而是任意大于1的整数$n$，某个$n$次单位根的$k$次方可以表示为$\zeta_m = e^{2 \pi m k i / n}$，因此可能存在某个$k < n$，使得$\zeta_m^k = 1$，只要$mk$是$n$的倍数即可。但如果$n$是素数$p$，则$k$一定是$p$的整数倍}）。我们记：
