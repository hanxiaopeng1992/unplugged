...
The corresponding example program is as below:
...
Here is the example program implements the traditional solution:
...
Where the folding starts from a tuple of three elements. The first two are pairs, represent the longest sub-string (its length and content), and the current sub-string. The last one in the tuple is the product of primes, starts from 1. Function $f$ is defined as the following:
...
(\mathbf{G} \circ \mathbf{F})\ id & = & \mathbf{G} (\mathbf{F}\ id) & \text{functor composition} \\
 & = & \mathbf{Maybe}\ id & \text{identity arrow for functor $\mathbf{F}$} \\
...
\begin{proof}
Let $p = left \circ f$，$q = right \circ g$
...
\item{In the Peano category $\pmb{Pno}$ (see the question 2 in exercise of section 1), what object in the form of $(A, f, z)$ is the initial object? What is the final object?}

The initial object is $(\pmb{N}, succ, 0)$. From it there is unique arrow to any object:
...
Observe the triangle of $D \times B$, $A \times B$, and $C$. We know the arrow of $D \times B \to A$ is $f \circ (g \times id)$.
...
No, such definition does not work. Consider carrier object $A$, Functor $\mathbf{NatF}$ is recursive, it does not send $A$ to a determined object. In fact, we expect it is mapped to a object in Peano category $(A, z, f)$.
...
First for the list concatenate $concat$:
...
First, we can transform the ZF-expression to $filter$, and combine the two round of list filter to one pass:
...
Next we can simplify the list concatenation:
...
\item{Verify the type constraint with category theory. Hint: consider the type of the catamorphism.}
...
\begin{enumerate}
\item Insert nothing. It means append $d$ to the last factor of the last sub-expression of $e_i$. Combine $f_n ++ [d]$ as a new factor. For example when $e_i$ is $1 + 2$, $d$ is 3, write 3 after $1 + 2$ without inserting any symbols, we obtain a new expression $1 + 23$;

\item Insert $\times$. It means we create a new factor $[d]$, then append it to the last sub-expression of $e_i$. Combine $t_m ++ [[d]]$ as a new sub-expression. For the same $1 + 2$ example, we write 3 after it, put a $\times$ between 2 and 3, hence obtain a new sub-expression $1 + 2 \times 3$;

\item Insert +. It means we create a new sub-expression $[[d]]$, then append it to $e_i$ to obtain a new expression $e_i ++ [[[d]]]$. For the same $1 + 2$ example, we write 3 after it, put a + between 2 and 3, hence obtain a new expression $1 + 2 + 3$.
\end{enumerate}
...
\item{The following definition convert expression to string:
\[
str = (join\ \text{``+''}) \circ (map\ ((join\ \text{``} \times \text{''}) \circ (map\ (show \circ dec))))
\]
Where $show$ convert number to string. Function $join(c, s)$ concatenates multiple strings $s$ with delimiter $c$. For example: $join($``\#''$, [$``abc'', ``def''$]) = $``abc\#def''. Use the fusion law to optimize $str$.
}
...
where $c = `+', d = ` \times '$, and $g = show \circ dec$. What we need to do is to simplify $(join\ c) \circ (map\ f)$.
...
Our idea is to map every unique character to a prime number, for example, a $\to$ 2, b $to$ 3, c $\to$ 5, ... Given any string $W$, no matter it contains repeated characters or not, we can convert it to a product of prime numbers:

\[
F = \prod p_c , c \in W
\]

We call it the number theory finger print $F$ of string $W$. We $W$ is empty, we define its finger print as 1. Because multiplication of integers is commutative, the number theory finger print is same for all permutations of $W$, and according to the fundamental theorem of arithmetic, the number theory finger print is unique. We can develop an elegant solution based on this: First, we calculate the number theory finger print $F$ of string $W$, then slide a window of length $|W|$ along $T$ from left to right. When start, we also need to compute the number theory finger print within this window of $T$, and compare it with $F$. If they are equal, it means $T$ contains some permutation of $W$. Otherwise, we slide the window to the right by a character. We can easily compute the updated number theory finger print value for this new window position: divide the product by the corresponding prime number for the character slide out, and multiply the prime number for the character slide in. Whenever the number theory finger print equals to $F$, we find a permutation. In order to map different characters to prime numbers, we can use sieve of Eratosthenes to generate a series of prime numbers. Below is the example algorithm accordingly.
...
\item{``The only constant is change'' said by Heraclitus. Is this a Russell's paradox?}

Yes, this is an instance of Russell's paradox.

\item{Is the quote saying by Socrates (the beginning of chapter 7) a Russell's paradox?}
...
\item{Prove the associate law of addition with TNT reasoning rules.}
