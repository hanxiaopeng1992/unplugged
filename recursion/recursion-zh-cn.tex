...
所谓扩展欧几里得算法，就是除了求得两个量$a$、$b$的最大公度$g$外，同时找到满足贝祖等式$ax + by = g$的两个整数$x$和$y$。为什么贝祖等式（Bézout's identity）\footnote{贝祖等式，或称贝祖定理，最早是由法国数学家梅齐里亚克（Claude Gaspard Bachet de Méziriac，1581–1638）发现并证明了其对整数成立，法国数学家贝祖（Étienne Bézout，1730–1783）证明这一等式对多项式成立。贝祖等式可以推广到任意的整环和主理想环（PID，Principle Ideal Domain）上。}一定成立呢？我们下面给出贝祖等式的一种证明。由$a$、$b$可以构造一个集合，包含所有它们正的线性组合：

\[
S = \{ ax + by | x, y \in \mathbb{Z} \text{且} ax + by > 0\}
\]

对于线段来说，$S$一定不为空，因为它至少包含$a$（此时$x = 1, y = 0$）和$b$（此时$x = 0, y = 1$）。有所$S$的所有元素都为正，所以它一定存在一个最小的元素，我们将其记为$d = as + bt$。我们接下来要证明$d$就是$a$和$b$的最大公度。为此我们将$a$表示成$d$的商与余数的形式。

\be
a = qd + r
\label{eq:Euclidean-division-ad}
\ee

其中余数$0 \leq r < d$。余数要么为0，要么在集合$S$中。这是因为

\[
\begin{array}{rll}
r & = a - qd & \text{由式(\ref{eq:Euclidean-division-ad})} \\
  & = a - q(as + bt) & d\text{的定义} \\
  & = a(1 - qs) - bqt & \text{合并整理}
\end{array}
\]

即$r$可以表示为$a$、$b$的线性组合，因此如果它不为0，则一定在集合$S$中。但这是不可能的，因为我们之前假设$d$是$S$中的最小正元素，而$r$却比$d$更小，这样就会产生矛盾。因此我们的得知$r$一定等于0。根据式(\ref{rq:Euclidean-division-ad})，$d$一定可以度量$a$。用同样的方法，可以证明$d$也一定可以度量$b$。因此$d$是它们的公度。接下来我们证明$d$是最大公度。令$c$为$a$和$b$的任意公度，根据定义，存在整数$m$、$n$使得$a = mc$、$b = nc$。这样$d$就可以表示为：
...
这说明$c$可以度量$d$，也就是说$c \leq d$。这就证明了$d$是最大公度。

扩展欧几里得算法(CLRS P860，此处使用构造性证明，以后再提供给予抽象代数的存在性证明。)、倒水趣题。
