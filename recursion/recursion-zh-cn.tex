...
因此给定两个瓶子的容量，我们立即能够判断是否可以得到体积为$d$的水——只要$d$能够被$g$度量\footnote{如果容积为整数，当且仅当$d$能够被最大公约数$g$整除。}。
...
在这一系列操作中，倒满$B$共3次，倒空$A$共1次。这一过程可以描述如下。

重复$y$次：
\begin{enumerate}
\item 倒满$B$；
\item 将$B$倒入$A$，若$A$变满，则将其倒空。
\end{enumerate}

因此剩下的唯一问题是寻找整数$x$和$y$。使得$ax + by = d$，根据扩展欧几里得算法，我们可以找到满足贝祖等式的一组解$ax_0 + by_0 = g$。因为$d$是最大公度$g$的$m$倍，我们只要把$x_0$和$y_0$相应加大$m$倍即可得到一组特解：
