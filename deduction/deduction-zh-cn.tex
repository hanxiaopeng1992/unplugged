...
许多列表相关的操作都可以用叠加来定义。例如累加：

\[
sum = foldr\ (+)\ 0
\]

前面提到的$and$函数，计算一个布尔值列表中的所有元素的逻辑与：
...
\[
filter\ f\ xs = foldr\ (x\ ys \mapsto if\ f(x)\ then\ x:ys\ else\ ys)\ []\ xs
\]
...
接着，如果用另一个起始值$z$和二元组合运算$\oplus$，对这一列表进行叠加，其结果就相当于用$z$替换[]，用“$\oplus$”替换“:”，然后直接调用过程$g$。

\be
foldr((\oplus), z, build(g)) = g((\oplus), z)
\ee
