...
Here is the corresponding example Haskell program. It uses tail recursion for performance purpose.
...
Observer this definition, and compare it with the list mapping definition in chapter 1, we can find they are exactly same except for the name. It means we can re-use the list mapping to define list functor. This is the case in Haskell, which re-uses \texttt{map} for list functor.
...
\begin{center}
Cartesian product $A \times B$ \quad \quad \quad disjoint union $A + B$
\end{center}
...
The detailed proof can be found in the appendix of this book. In some program environment, like Haskell, product is realized with paired tuple $(a, b)$ and functions $fst, snd$. However, coproduct is realized with a dedicated data type.
