...
由于历史的原因，我们用同一个符号$\mathbf{F}$既表示对象间的变换，也表示箭头间的变换。注意这是一个非斜体的粗体大写字母，请注意它和范畴（粗斜体大写）、对象（斜体大写）所用字体的差别。有些编程环境，如Haskell，使用不同的记号来表示函子中对象的变换和箭头的变换。我们稍后在例子中会看到这些不同点。
...
2015年后主流的编程环境都纷纷将Haskell中的Maybe概念引入以取代null来获取更安全的方法\footnote{例如Java和C++中的\texttt{Optional<T>}。}。
...
例如有一个元素类型为$A$的二叉搜索树，在其中搜索一个值时，可能会搜不到。所以搜索的结果类型是$\mathbf{Maybe}A$。
...
这种给定$f$，映射成$f'$的行为恰好就是$\mathbf{Maybe}$函子对箭头进行的映射。在Haskell中，通常使用\texttt{fmap}来定义函子对箭头的映射：
...
也就是说，如果\texttt{F}是一个函子，它把从从\texttt{A}到\texttt{B}的箭头映射成从\texttt{F A}到\texttt{F B}的箭头。因此对于$\mathbf{Maybe}$函子，相应的\texttt{fmap}定义如下：
...
n < 2: & n \\
\text{否则}: & binary(\lfloor\dfrac{n}{2}\rfloor)(n \bmod 2)
\end{cases}
\]
...
\begin{tikzpicture}
  \matrix (m) [matrix of math nodes,
     ...
     \texttt{Maybe Int} & \texttt{Maybe [Int]} \\
     \texttt{Int} &  \texttt{[Int]} \\};
  \path[-stealth]
    (m-1-1) edge node [above] {\texttt{fmap binary}} (m-1-2)
    (m-2-1) edge node [below] {\texttt{binary}} (m-2-2)
    (m-2-1) edge (m-1-1)
    (m-2-2) edge (m-1-2);
\end{tikzpicture}
\end{center}
