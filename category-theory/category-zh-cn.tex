...
\subsubsection{积函子}
有了二元函子的概念，就可以定义积函子了。我们用中缀的形式，将积函子记为“$\times$”。对于两个对象，积函子将其映射为对象的积；对于箭头，定义：
...
\[
\begin{array}{rcll}
id \times id & = & \langle id \circ fst, id \circ snd \rangle & \text{$\times$的定义} \\
  & = & \langle fst, snd \rangle & \text{$id$的性质} \\
  & = & id & \text{积的反射律} \\
\end{array}
\]

其次验证组合性质$(f \times g) \circ (f' \times g') = (f \circ f') \times (g \circ g')$。为此，我们先证明积的\textbf{吸收律}（absorption law）：
...
\[
\begin{array}{rcll}
(f \times g) \circ \langle p, q \rangle & = & \langle f \circ fst, g \circ snd \rangle \circ \langle p, q \rangle & \text{$\times$的定义} \\
...
\[
\begin{array}{rcll}
(f \times g) \circ (f' \times g') & = & (f \times g) \circ \langle f' \circ fst, g' \circ snd \rangle & \text{对第二项用$\times$的定义} \\
...
积函子是二元函子的一个特例。与$fmap$类似，在Haskell中，专门为二元函子定义了一个$bimap$。
