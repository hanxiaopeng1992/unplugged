...
第一种箭头是说，两个幺半群的元素进行二元运算，其结果仍是这个幺半群中的元素（二元运算是封闭的）；第二种箭头是“选择函数”，1是终止对象\footnote{我们借鉴Haskell中符号，特意用“()”表示它，在同构的意义下$1 = \{ \bigstar \} = \{ () \}$}。
...
全部组成幺半群的箭头的集合，也就是这两种幂对象的积

\[
M^{M \times M} \times M^1 = M^{M \times M + 1}
\]

我们再把右侧从幂对象写回箭头的形式，这样从任意集合$M$，产生幺半群的（类型）函子一定是下面的箭头：
