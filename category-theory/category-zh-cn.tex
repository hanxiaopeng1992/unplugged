...
看成对象。在上下文清楚的情况下，我们通常用二元组$(A, \alpha)$表示对象。
两个对象间的箭头定义如下：
...
\item 箭头$\mathbf{NatF}(\mathbf{NatF}I) \to \mathbf{NatF}I$。我们知道函子既作用于对象，也作用于箭头。所以我们可以通过函子$\mathbf{NatF}$，将初始对象中的箭头$i$“举”上去。因此，这个递归的F-代数的箭头就是$\mathbf{NatF}(i)$。
\end{enumerate}

这样，这个递归的F-代数可以记为：$\mathbf{NatF}-alg(\mathbf{NatF}I, \mathbf{NatF}(in))$。因为从初始对象到它也有唯一的箭头，所以存在$I \arrowto{k} \mathbf{NatF}I$，使得下面的范畴图可交换：
