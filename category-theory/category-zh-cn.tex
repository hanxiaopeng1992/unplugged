...
正如迪厄多内所说：“这种抽象绝不是来自数学家的反常意愿，似乎他们想通过使用深奥莫测的语言来把自己与其它人隔开。数学家是被经典对象和关系的本质特性逼着去锻造新的抽象工具，来解决过去看来是不可攻克的问题。”\cite{Dieudonne1987}（中文版第2页，英文版第9页）
...
\[
\mathbf{F}(f) \circ \phi_B = \phi_A \circ \mathbf{G}(f)
\]
...
\begin{tikzpicture}
  \matrix (m) [matrix of math nodes,
               row sep=3em, column sep=5em, minimum width=2em]{
     A & A \times B \arrowto{h} \to C \\
     D & D \times B \arrowto{k} \to C \\};
  \path[-stealth]
    (m-1-1) edge node [left] {$f$} (m-2-1)
...
如果在$\pmb{C}$中存在$A \arrowto{f} D$的箭头，则$\pmb{Exp}$中的箭头为$h \longrightarrow{j} k$。当且仅当把上面范畴图中右侧的$C$合并时，箭头之间可交换：
...
这恰好是前面介绍过的“\hyperref[sec:selection-arrow]{选择箭头}”。1是终止对象，所以幂$A^1$表示了从终止对象到到$A$的箭头集合$\{ 1 \to A\}$。如果$A$是一个集合，我们可以针对集合中的任何元素$a \in A$构造一个从终止对象1到$a$的一个函数：
...
\[
  F-alg (A, \alpha) \quad \quad \quad F-coalg (A, \alpha)
\]
...
F-态射：
\[
  F-alg (A, \alpha) \longrightarrow F-alg (B, \beta)
  \quad \quad \quad
  F-coalg (A, \alpha) \longrightarrow F-coalg (B, \beta)
\]
...
假设有一个集合$A$，具有自然数的代数结构。还记得第一章中斐波那契数列的例子么？具体到这个例子，$A$就是数对$(Int, Int)$。起始对象是$(1, 1)$，后继函数是$h(m, n) = (n, m + n)$。
...
这个函子是一个多项式函子$\mathbf{NatF} A = 1 + A$。箭头$\alpha$是函数$zero, succ$的余积$[zero, succ]$。

现在我们提出一个问题。如果令$A’ = \mathbf{NatF} A$，将其代入$\mathbf{NatF}$函子，那么$\mathbf{NatF} A'$是什么？应该是两重函子$\mathbf{NatF}(\mathbf{NatF} A)$。我们还可以重复这一过程得到三重的$\mathbf{NatF}(\mathbf{NatF}(\mathbf{NatF} A))$。我们可以重复无穷多次，现在我们给重复无穷多次后产生类型$\mathbf{NatF}(\mathbf{Nat}(...))$起名为$\mathbf{Nat}$，即：
...
这恰恰是我们在第一章定义的自然数类型函子。我们把\texttt{Nat}和\texttt{Nat A}并列写在一起：
...
自函子$\mathbf{F}$的不动点是$\mathbf{Fix}\ \mathbf{F}$，将$\mathbf{F}$应用到不动点上，得到的仍然是不动点。我们可以定义一对函数，操作不动点：
...
因此，$\mathbf{Nat}$是函子$\mathbf{NatF}$的不动点。
...
\end{array}
\]
如果迭代地将$A' = \mathbf{NatF} A$代入$\mathbf{NatF}$函子$n$次。我们把这样得到的函子记为$\mathbf{NatF}^n A$。试定义下面的$\alpha$箭头：
\[
\begin{array}{l}
...
在F-代数组成范畴$\pmb{Alg}(F)$中，如果存在初始对象，它会有什么特殊的性质呢？初始对象到其它对象的唯一箭头表示什么样的关系呢？F-代数范畴中的所有对象形如$F-alg(A, \alpha)$，我们用类自然数的F-代数来举例。在$\pmb{Alg}(NatF)$中，所有的对象都表示为$NatF-alg(A, \alpha)$。其中$NatF$的定义如同上小节；携带对象是$A$，箭头是$\mathbf{NatF} A \arrowto{\alpha} A$。

再具体些，F-代数$NatF-alg(Int, [zero, succ])$是一个对象，其中$zero, succ$的定义为：
...
它们的余积$\alpha = [zero, succ] = zero + succ$，我们将其命名为$eval$,它的定义为：
...
另一个具体对象是F-代数$NatF-alg(Int \times Int, fib)$，其中$\mathbf{NatF} (Int \times Int) \arrowto{fib} (Int \times Int)$定义为：
...
如果范畴$\pmb{NatF}$有起始对象，则它一定具有形式$0 = \mathbf{NatF}-alg(I, in)$。因为起始对象到任何对象都存在唯一的箭头。所以它到$\mathbf{NatF}-alg(A, \alpha)$一定有唯一的箭头。即存在箭头$I \arrowto{f} A$，使得下面的范畴图可交换\footnote{起始的英文是initial，所以选择符号$(I, in)$代表起始对象中的集合$I$和箭头$in$}：
...
\begin{tikzpicture}
  ...
  \path[-stealth]
    ...
    (m-2-1) edge node [below] {$in$} (m-2-2);
\end{tikzpicture}
\end{center}

因为起始对象到\textbf{任何}对象都有唯一箭头，所以它到自己的递归——由$\mathbf{NatF}(\mathbf{NatF}A)$等——构成的对象也一定有唯一的箭头。具体来说这个递归的F-代数具有的三要素是：

\begin{enumerate}
\item 共同的函子$\mathbf{NatF}$；
\item 携带对象是$\mathbf{NatF}I$；
\item 箭头$\mathbf{NatF}(\mathbf{NatF}I) \to \mathbf{NatF}I$。我们知道函子既作用于对象，也作用于箭头。所以我们可以通过函子$\mathbf{NatF}$，将初始对象中的箭头$in$“举”上去。因此，这个递归的F-代数的箭头就是$\mathbf{NatF}(in)$。
\end{enumerate}

这样，这个递归的F-代数可以记为：$NatF-alg(\mathbf{NatF}I, \mathbf{NatF}(in))$。因为从初始对象到它也有唯一的箭头，所以存在$I \arrowto{k} \mathbf{NatF}I$，使得下面的范畴图可交换：
...
\begin{tikzpicture}
  \matrix (m) [matrix of math nodes,
    ...
    \mathbf{NatF} I  & I \\};
  \path[-stealth]
    (m-1-1) edge node [above] {$\mathbf{NatF}(in)$} (m-1-2)
    (m-2-1) edge node [left] {$\mathbf{NatF}(k)$} (m-1-1)
    (m-2-2) edge node [right] {$k$}   (m-1-2)
    (m-2-1) edge node [below] {$in$} (m-2-2);
\end{tikzpicture}
\end{center}
...
\[
  \mathbf{NatF}(\mathbf{NatF}I) \arrowto{\mathbf{NatF}(in)} \mathbf{NatF} I \arrowto{in} I
\]
...
\begin{tikzpicture}
  ...
  \path[-stealth]
    (m-1-1) edge node [above] {$\mathbf{NatF}(in)$} (m-1-2)
    (m-2-2) edge node [right] {$in$}   (m-1-2);
\end{tikzpicture}
\end{center}

和上面的范畴图对边，我们发现$in$和$k$的方向相反。如果把它们两个组合起来，就得到了一个从$I$出发指回自己的箭头。并且由于起始对象到自己的唯一箭头就是$id$，所以：

\[
in \circ k = id
\]

同样的分析对$\mathbf{NatF}I$也成立，因为起始对象到递归对象的箭头也是唯一的。所以从$\mathbf{NatF}I$出发到$I$的箭头$in$，然后再从箭头$k$返回$\mathbf{NatF}I$必然也是$id$：

\[
k \circ in = id_{\mathbf{NatF}I}
\]
...
这一结论，和1889年皮亚诺给出算术公理时的结论完全一样，任何满足皮亚诺公理的结构$(A, [e, f])$，都存在从自然数$(N, [zero, succ])$到这一结构的唯一同构。它将自然数$n$映射到$f^n(e) = f(f(...f(e))..)$上。现在，我们可以给出\textbf{初始代数}（initial algebra）的定义了。

\begin{definition}
F-代数范畴$\pmb{Alg}(F)$中，如果存在初始对象，则这一初始对象叫做初始代数。
\end{definition}

在集合全函数范畴中，许多函子，包括多态函子都存在初始代数。我们略去了初始代数的存在性证明，读者可以参考\cite{Manes-Arbib-1986}。给定函子$\mathbf{F}$我们可以用上一小节定义的不动点函数得到这一F-代数中的初始对象：
...
1968年，兰贝克（Lambek）最早指出$in$是一个同构映射，并且称初始代数$(I, in)$是函子$\mathbf{F}$的不动点\cite{Lambek-1968}。现在这一事实被称作\textbf{兰贝克定理}。
