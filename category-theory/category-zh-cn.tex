...
我们举一些具体的例子来帮助理解自然变换。第一个例子是前缀枚举函数$inits$。任给一个字符串或者列表，$inits$可以枚举出所有的前缀，例如：
...
我们举的第二个例子叫做$safeHead$

我们说自然变换是被$A$索引的，在编程环境中，这相当于说给定类型


我们说自然变换是用来比较函子的。仿照抽象代数中同构的含义，我们需要定义什么情况下我们认为两个函子是“自然同构”的。
...
有时我们也说两个自然同构的函子是自然等价（natually equivalent）的。
