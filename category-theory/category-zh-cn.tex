...
范畴中的某些箭头具有有一些特殊的性质，我们这一节介绍一对这样的对偶性质，称为左消去\footnote{也有人译为“首一”，但是这里译为消去更易理解。}（monic）和右消去（epic）。既然是对偶的性质，我们就用对偶的形式给出：
...
\begin{tikzpicture}
  ...
  \path[-stealth]
    ...
    (m-1-4.south east) edge node [above] {$g$} (m-1-5.south west);
\end{tikzpicture}
\end{center}
...
左右可消去箭头有一些有趣的性质，我们以左消去为例，读者可以尝试用对偶的方式思考右消去的情形。
