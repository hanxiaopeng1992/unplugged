...
\[
Fix\ \mathbf{F} = \mathbf{F}\ (Fix\ \mathbf{F})
\]

自函子$\mathbf{F}$的不动点是$Fix\ \mathbf{F}$，将$\mathbf{F}$应用到不动点上，得到的仍然是不动点。我们可以定义一对函数，操作不动点：

\[
\begin{array}{l}
fix : \mathbf{F}\ (Fix\ \mathbf{F}) \to Fix\ \mathbf{F} \\
fix\ \mathbf{F} = let\ X = \mathbf{F} X\ in\ X \\
\\
unfix : (Fix\ \mathbf{F}) \to \mathbf{F}\ (Fix\ \mathbf{F}) \\
unfix\ (Fix\ X) = X \\
\end{array}
\]
